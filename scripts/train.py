...
1) Generate a set of "gentle" paths (from mpc_net/paths/...)
2) Collect single-step MPC data (from mpc_net/training/data_collection)
3) Train a Mixture-of-Experts model (mpc_net/training/train_moe)
4) Save it as 'moe_model.keras', ensuring we can reload it immediately
"""
...
from mpc_net.paths.path_generators import generate_paths
...
def main():
    ...
    # 2) Generate gentle paths
    path_gens = generate_paths()
    ...
    plt.grid(True)
    plt.legend()
    plt.title("MPC-Collected Trajectories on Gentle Paths")
    plt.show()

    # 5) Train or Load
    model_path = "moe_model.keras"   # use .keras (preferred Keras format)
    ...
    if os.path.exists(abs_path):
        ...
        # 6) Save in new .keras format
        print(f"\nSaving new model to: {abs_path}")
        model.save(abs_path)
        print("DEBUG: Model saved.")

        # 7) Immediately reload to confirm it matches
